...
# === Status Indicator Overlay (Green Checkmark on Secondary Monitor) ===
def create_status_overlay():
    """Create a green checkmark overlay on secondary monitor bottom right corner."""
    try:
        ...
        if not secondary:
            print("[StatusOverlay] No secondary monitor found, skipping overlay")
            return None
        
        # Create overlay window
        overlay = tk.Tk()
        overlay.title("Status Indicator")
        overlay.overrideredirect(True)  # Remove window decorations
        overlay.attributes('-topmost', True)  # Always on top
        overlay.attributes('-alpha', 0.95)  # Mostly opaque (was 0.85, making it more visible)
        overlay.configure(bg='#00FF00')  # Green background for the window itself
        
        # Set window size (make it a bit bigger so it's more visible)
        window_width = 100
        window_height = 100
        
        # Calculate position: bottom right of secondary monitor
        pos_x = secondary.x + secondary.width - window_width - 20  # 20px margin from edge
        pos_y = secondary.y + secondary.height - window_height - 20  # 20px margin from edge
        
        overlay.geometry(f"{window_width}x{window_height}+{pos_x}+{pos_y}")
        
        # Create green background with checkmark
        canvas = tk.Canvas(
            overlay,
            width=window_width,
        ...
        # Draw a large checkmark using lines
        # Checkmark shape: two lines forming a check
        check_size = 50  # Made bigger for visibility
        ...
        # Alternative: Use Unicode checkmark character (simpler)
        # canvas.create_text(
        #     center_x, center_y,
        #     text="✓",
        #     font=("Arial", 50, "bold"),
        ...
        # Don't make it click-through - keep it visible and clickable
        # (Click-through would make it invisible)
        
        print(f"[StatusOverlay] ✅ Green checkmark overlay created")
        print(f"[StatusOverlay] Position: ({pos_x}, {pos_y}) on secondary monitor")
        print(f"[StatusOverlay] Secondary monitor: X={secondary.x}, Y={secondary.y}, W={secondary.width}, H={secondary.height}")
        ...
        print(f"[StatusOverlay] ❌ Failed to create overlay: {e}")
        import traceback
        traceback.print_exc()
        return None
...
def run_status_overlay():
    """Run the status overlay in a separate thread."""
    try:
        # Small delay to ensure monitors are detected
        time.sleep(1.0)
        overlay = create_status_overlay()
        if overlay:
            print(f"[StatusOverlay] Starting overlay mainloop...")
            overlay.mainloop()
        else:
            print(f"[StatusOverlay] Overlay creation returned None")
    except Exception as e:
        print(f"[StatusOverlay] Error in run_status_overlay: {e}")
        import traceback
        traceback.print_exc()
...
# Additional imports for print server
try:
    import win32print
    import win32api
    import win32ui
    ...
    win32print = win32api = win32ui = win32con = None
...
# Enable CORS for print server endpoints
CORS(app, resources={
    r"/*": {
        "origins": [
            "http://localhost:3000",
...
# ================================
# SECTION: Print Server Routes (from JRCO_PrintServer.py)
# ================================

# Print server constants
# Try to find SumatraPDF in common locations
def find_sumatra_path():
    """Find SumatraPDF executable in common locations."""
    possible_paths = [
        r"C:\Users\eckar\AppData\Local\SumatraPDF\SumatraPDF.exe",
...
SUMATRA_PATH = find_sumatra_path()
if SUMATRA_PATH:
    print(f"[Print] SumatraPDF found at: {SUMATRA_PATH}")
else:
    print(f"[Print] SumatraPDF not found, will use Windows default print")
WATCH_FOLDER = r"G:\My Drive\Label Printer"
PRINTER_NAME = "PL80E (1)"
...
def find_matching_printer(printer_name):
    """Find the best matching printer name, trying exact match first, then partial matches."""
    if not win32print:
        return printer_name
    ...
    for printer in available:
        if printer.lower() == printer_name.lower():
            return printer
    
    # Try partial match (printer_name in available printer name)
    for printer in available:
        if printer_name.lower() in printer.lower():
            return printer
    
    # Try reverse partial match (available printer name in printer_name)
    for printer in available:
        if printer.lower() in printer_name.lower():
            return printer
    
    return printer_name

def find_latest(order, suffix):
    """Find the latest PDF file matching the pattern."""
...
def silent_print(filepath, printer_name=None):
    """Print a PDF file to specified printer by converting to image and printing."""
    # Use specified printer or fall back to PRINTER_NAME constant or default
    target_printer = printer_name or PRINTER_NAME or (win32print.GetDefaultPrinter() if win32print else None)
    
    if not target_printer:
        print(f"[Print] No printer specified, cannot print")
        return
    
    print(f"[Print] Using printer: {target_printer}")
    
    # Method 1: Try SumatraPDF first (best option - silent printing)
    if SUMATRA_PATH and os.path.exists(SUMATRA_PATH):
        try:
            ...
            print(f"[Print] ✅ Printed via SumatraPDF to {target_printer}")
            return
        except Exception as e:
            print(f"[Print] SumatraPDF failed: {e}, trying image conversion method...")
    
    # Method 2: Convert PDF to image and print using win32ui (same as label printing)
    try:
        if fitz is None:
            print(f"[Print] PyMuPDF (fitz) not available, cannot convert PDF to image")
            raise Exception("fitz not available")
        
        # Convert PDF first page to high-res image
        pdf = fitz.open(filepath)
        page = pdf[0]
        # Use high resolution (2x for good quality)
        pix = page.get_pixmap(matrix=fitz.Matrix(2, 2))
        
        # Create temporary PNG
        import tempfile
        temp_dir = tempfile.gettempdir()
        temp_png = os.path.join(temp_dir, f"print_temp_{int(time.time())}.png")
        pix.save(temp_png)
        pdf.close()
        
        # Load as PIL Image
        img = PILImage.open(temp_png).convert("RGB")
        
        # Print using win32ui (same method as label printing)
        ...
        print(f"[Print] ✅ Printed PDF to {target_printer} (converted to image)")
        return
        
    except Exception as e:
        print(f"[Print] Image conversion method failed: {e}")
    ...
    try:
        ...
        result = subprocess.run(
            ['powershell', '-Command', f'Start-Process -FilePath "{filepath}" -Verb Print -ArgumentList @(\'"{target_printer}"\')'],
            check=False,
            timeout=30
        )
        if result.returncode == 0:
            print(f"[Print] ✅ Sent to printer via PowerShell: {target_printer}")
            time.sleep(3)
            return
    except Exception as e:
        print(f"[Print] PowerShell print failed: {e}")
    
    # Method 4: Try os.startfile with printto
    try:
        # Use printto verb with specific printer
        os.startfile(filepath, "printto")
        print(f"[Print] ✅ Sent print command via os.startfile: {target_printer}")
        time.sleep(3)
        return
    except Exception as e:
        print(f"[Print] os.startfile printto failed: {e}")
    
    # Final fallback: Try default print handler
    try:
        os.startfile(filepath, "print")
        print(f"[Print] ⚠️ Sent to default printer (may not be {target_printer})")
        time.sleep(3)
    except Exception as e:
        print(f"[Print] ❌ All print methods failed: {e}")
...
@app.post("/print")
def print_files():
    """Print stamped PDFs or binsheets for an order."""
...
@app.post("/queue-emb")
def queue_emb():
    """Queue an embroidery file in Wilcom."""
...
# ================================
# SECTION: UPS Label Watcher (from JRCO_PrintServer.py)
# ================================
...
def print_label(file_path):
    """Print a UPS label with cropping, rotation, and resizing."""
    try:
        print("\n==============================")
        ...
        if fitz is None:
            print("❌ Error: PyMuPDF (fitz) not installed. Cannot convert PDF to PNG.")
            return

        folder = os.path.dirname(file_path)
        base = os.path.splitext(os.path.basename(file_path))[0]
        original_pdf_path = file_path  # Track original PDF path for cleanup
        ...
        if file_path.lower().endswith(".pdf"):
            ...
            try:
                ...
                pdf.close()  # Close PDF to release file lock
        ...
        # Try to find the best matching printer name
        actual_printer_name = find_matching_printer(PRINTER_NAME)
        if actual_printer_name != PRINTER_NAME:
            print(f"ℹ️ Using printer: {actual_printer_name} (matched from '{PRINTER_NAME}')")
        
        try:
            dc = win32ui.CreateDC()
            dc.CreatePrinterDC(actual_printer_name)
            PW = dc.GetDeviceCaps(win32con.HORZRES)     # e.g., 799
            PH = dc.GetDeviceCaps(win32con.VERTRES)     # e.g., 1199
            
            # Get printer DPI and physical paper size to diagnose settings
            dpi_x = dc.GetDeviceCaps(win32con.LOGPIXELSX)
            dpi_y = dc.GetDeviceCaps(win32con.LOGPIXELSY)
            paper_width_inches = dc.GetDeviceCaps(win32con.HORZSIZE) / 100.0  # mm to inches
            paper_height_inches = dc.GetDeviceCaps(win32con.VERTSIZE) / 100.0  # mm to inches
            
            print(f"Printer DPI: {dpi_x} × {dpi_y}")
            print(f"Paper size (reported): {paper_width_inches:.2f}\" × {paper_height_inches:.2f}\"")
            print(f"Expected for 4×6 label: 4.00\" × 6.00\" (portrait)")
            
            if abs(paper_width_inches - 4.0) > 0.5 or abs(paper_height_inches - 6.0) > 0.5:
                print(f"⚠️ WARNING: Printer paper size doesn't match 4×6 label!")
                print(f"   Please check printer settings and set paper size to 4×6 inches")
        except Exception as e:
            print(f"❌ Error creating printer DC for '{actual_printer_name}': {e}")
            print("\n📋 Available printers on this system:")
            available = list_available_printers()
            if available:
                for printer in available:
                    marker = " ← MATCH!" if PRINTER_NAME.lower() in printer.lower() or printer.lower() in PRINTER_NAME.lower() else ""
        ...
        print(f"Rotated image size: {rotated.size[0]} × {rotated.size[1]}")

        # Force resize to exact dimensions to fit on label (matches JRCO_PrintServer.py)
        final_img = rotated.resize((SAFE_W, SAFE_H), PILImage.LANCZOS)
        ...
        try:
            ...
            # Position at border (matches JRCO_PrintServer.py)
            left = BORDER
            top = BORDER
            right = SAFE_W + BORDER
            ...
            print("✅ PRINT COMPLETE — Sent label to printer")
        except Exception as e:
            print(f"❌ Error printing to '{actual_printer_name}': {e}")
            print("\n📋 Available printers on this system:")
            available = list_available_printers()
            if available:
                for printer in available:
        ...
        # STEP 6: Delete the printed file and any generated temp files
        print("🧹 Cleaning up printed file...")
        
        # Delete the original PDF, converted PNG, and final PNG
        files_to_delete = []
        if original_pdf_path and os.path.exists(original_pdf_path):
            files_to_delete.append(original_pdf_path)
            print(f"   Will delete original PDF: {os.path.basename(original_pdf_path)}")
        # file_path is the converted PNG (if it was a PDF originally)
        if file_path and file_path != original_pdf_path and os.path.exists(file_path):
            files_to_delete.append(file_path)
            print(f"   Will delete converted PNG: {os.path.basename(file_path)}")
        # final_path is the final processed PNG
        if final_path and os.path.exists(final_path):
            files_to_delete.append(final_path)
            print(f"   Will delete final PNG: {os.path.basename(final_path)}")
...
class LabelHandler(FileSystemEventHandler):
    """File system event handler for UPS label printing."""
...
def start_label_watcher():
    """Start watching the UPS label folder for new PDFs."""
    print(f"👀 Watching UPS Label folder: {WATCH_FOLDER}")
    print(f"🖨️ UPS Label Printer: {PRINTER_NAME}")

    if not os.path.exists(WATCH_FOLDER):
        print(f"⚠️ Warning: Label folder does not exist: {WATCH_FOLDER}")
        return

    # Print all existing PDF files in the folder before starting to watch
    printed_files = set()
    try:
        existing_files = []
        for fname in os.listdir(WATCH_FOLDER):
            file_path = os.path.join(WATCH_FOLDER, fname)
            if os.path.isfile(file_path) and file_path.lower().endswith(".pdf"):
                if ".tmp" not in file_path.lower() and "~" not in file_path.lower():
                    existing_files.append(file_path)
        
        if existing_files:
            print(f"📄 Found {len(existing_files)} existing PDF file(s) in folder, printing them now...")
            for file_path in existing_files:
                if file_path not in printed_files and os.path.exists(file_path):
                    try:
                        print_label(file_path)
                        ...
                        print(f"⚠️ Error printing {os.path.basename(file_path)}: {e}")
                        import traceback
                        traceback.print_exc()
                        # Don't add to printed_files if it failed, so it can be retried
            print("✅ Finished printing existing files")
    except Exception as e:
...
# ================================
# SECTION: Print Fur PDF Watcher
# ================================

PRINT_FUR_FOLDER = r"G:\My Drive\Print Fur PDF"
last_fur_print_time = {}

def print_fur_pdf_and_queue(file_path):
    """Print a PDF to default printer and queue the corresponding .emb file in Wilcom."""
    try:
        print("\n==============================")
        print("📄 PRINT FUR PDF AND QUEUE")
        print("==============================")
        print(f"Incoming file: {file_path}")
        ...
        # Assuming filename format like: "12345_Stamped.pdf" or "12345.pdf" or similar
        filename = os.path.basename(file_path)
        base_name = os.path.splitext(filename)[0]
        
        # Try to extract order number (first part before underscore, or whole name if no underscore)
        order_match = re.match(r'^(\d+)', base_name)
        if not order_match:
            # Try to find order number in the filename
            order_match = re.search(r'(\d{4,})', base_name)
        
        if not order_match:
            print(f"❌ Could not extract order number from filename: {filename}")
            return
        
        order = order_match.group(1)
        print(f"📋 Extracted order number: {order}")

        # STEP 0: Fill notes from Google Sheets (before printing)
        print(f"📝 Filling notes from Google Sheets...")
        try:
            notes_filled = fill_pdf_notes_from_sheets(file_path)
            if notes_filled:
                print(f"✅ Notes successfully filled in PDF")
            else:
                print(f"⚠️ Notes filling failed or no notes found (continuing anyway...)")
        except Exception as e:
            print(f"⚠️ Error filling notes: {e} (continuing with print/queue anyway...)")
            import traceback
            traceback.print_exc()

        # STEP 1: Print the PDF to specified printer
        print(f"🖨️ Printing PDF to printer: {PRINTER_NAME}...")
        try:
            silent_print(file_path)
            print(f"✅ PDF sent to printer")
        except Exception as e:
            print(f"⚠️ Error printing PDF: {e}")
            print(f"   Will continue with Wilcom queueing anyway...")

        # STEP 2: Find and open the .emb file in Wilcom (headless)
        emb_path = os.path.join(BASE_PATH, order, f"{order}.emb")
        if not os.path.exists(emb_path):
            print(f"❌ Error: .emb file not found at {emb_path}")
            return
        
        print(f"📂 Found .emb file: {emb_path}")
        
        # Check if Wilcom is already running
        import pygetwindow as gw
        wilcom_windows = gw.getWindowsWithTitle("EmbroideryStudio")
        wilcom_running = len(wilcom_windows) > 0
        
        if not wilcom_running:
            # Launch Wilcom minimized/headless
            print(f"🚀 Launching Wilcom (headless)...")
            try:
                # Try to find Wilcom executable in common locations
                wilcom_paths = [
                    r"C:\Program Files\Wilcom\EmbroideryStudio\EmbroideryStudio.exe",
                    r"C:\Program Files (x86)\Wilcom\EmbroideryStudio\EmbroideryStudio.exe",
                    r"C:\Program Files\Wilcom\EmbroideryStudio 2025\EmbroideryStudio.exe",
                    r"C:\Program Files (x86)\Wilcom\EmbroideryStudio 2025\EmbroideryStudio.exe",
                ]
                
                wilcom_exe = None
                for path in wilcom_paths:
                    if os.path.exists(path):
                        wilcom_exe = path
                        break
                
                if not wilcom_exe:
                    # Try to find it via file association (open a dummy .emb to get the exe path)
                    # For now, just try to open Wilcom via start menu or use the .emb file association
                    # Launch minimized using ShellExecute with SW_MINIMIZE
                    if win32api:
                        # SW_MINIMIZE = 6, SW_SHOWMINIMIZED = 2
                        # Try to launch via start menu or registry
                        win32api.ShellExecute(0, "open", "EmbroideryStudio.exe", None, None, 6)  # SW_MINIMIZE
                    else:
                        subprocess.Popen(["EmbroideryStudio.exe"], creationflags=subprocess.CREATE_NO_WINDOW)
                else:
                    # Launch the executable minimized
                    if win32api:
                        win32api.ShellExecute(0, "open", wilcom_exe, None, None, 6)  # SW_MINIMIZE
                    else:
                        subprocess.Popen([wilcom_exe], creationflags=subprocess.CREATE_NO_WINDOW)
                
                # Wait for Wilcom to start
                print(f"⏳ Waiting for Wilcom to start...")
                time.sleep(3)
                
                # Verify Wilcom is running
                wilcom_windows = gw.getWindowsWithTitle("EmbroideryStudio")
                if not wilcom_windows:
                    print(f"⚠️ Warning: Wilcom may not have started, will try to continue...")
                else:
                    # Keep it visible for debugging
                    try:
                        wilcom_windows[0].restore()
                    except:
                        pass
                    print(f"✅ Wilcom is running (visible)")
            except Exception as e:
                print(f"⚠️ Warning: Could not launch Wilcom: {e}")
                print(f"   Will try to open file directly...")
                # Fallback: open file directly
                if win32api:
                    win32api.ShellExecute(0, "open", emb_path, None, ".", 6)  # SW_MINIMIZE
                else:
                    os.startfile(emb_path)
                time.sleep(2)
        else:
            print(f"✅ Wilcom is already running")
            # Keep it visible for debugging
            try:
                wilcom_windows[0].restore()
            except:
        ...
        # Now open the file from within Wilcom using File > Open (Ctrl+O)
        print(f"📂 Opening file in Wilcom...")
        try:
            # Connect to Wilcom
            app = Application(backend="uia").connect(title_re=EMB_WINDOW_TITLE, timeout=5)
            win = app.window(title_re=EMB_WINDOW_TITLE)
            
            # Activate window and keep it visible
            try:
                win.set_focus()
                win.restore()  # Make sure it's visible
            except:
                pass
            ...
            from pywinauto.keyboard import send_keys
            send_keys('^o')  # Ctrl+O for File > Open
            time.sleep(0.8)  # Wait for dialog to appear
            
            # Use clipboard to paste the file path (more reliable than typing)
            try:
                import pyperclip
                old_clipboard = pyperclip.paste()
                pyperclip.copy(emb_path)
                time.sleep(0.2)
                ...
                # Fallback: type the path directly
                # First, clear any existing text (Ctrl+A, Delete)
                send_keys('^a')
                time.sleep(0.1)
                send_keys(emb_path)
                time.sleep(0.2)
            ...
            # Press Enter to open
            send_keys('{ENTER}')
            time.sleep(2.0)  # Wait for dialog to close and file to start loading
            
            # Close the connection - we'll reconnect after file loads
            app = None
            win = None
            
        except Exception as e:
            print(f"⚠️ Warning: Could not open file via File > Open: {e}")
            print(f"   Falling back to direct file open...")
            # Fallback: open file directly (will open in new window)
            if win32api:
                win32api.ShellExecute(0, "open", emb_path, None, ".", 6)  # SW_MINIMIZE
            else:
                os.startfile(emb_path)
            time.sleep(3)  # Wait longer for direct open

        # STEP 3: Wait for file to load and find the correct window using pywinauto
        print(f"⏳ Waiting for file to load in Wilcom...")
        max_wait = 30
        file_loaded = False
        win = None
        ...
        while time.time() - start_time < max_wait:
            try:
                # Connect to Wilcom and check all windows
                app_check = Application(backend="uia").connect(title_re=EMB_WINDOW_TITLE, timeout=2)
                all_wins = app_check.windows()
                
                # Check each window's title for the order number
                for w in all_wins:
                    try:
                        window_text = w.window_text() or ""
                        # Check if order number or filename is in the title
                        if order in window_text or os.path.basename(emb_path).lower().replace('.emb', '') in window_text.lower():
                            # Found the window with our file!
                            win = w
                            file_loaded = True
                            print(f"✅ File loaded in Wilcom (found window: {window_text[:70]}...)")
                            break
                    except:
                        pass
        ...
        if not file_loaded:
            print(f"⚠️ Warning: Could not verify file loaded after {max_wait}s")
            print(f"   Will try to queue on any Wilcom window...")
            # Try to connect to any Wilcom window
            try:
                app_check = Application(backend="uia").connect(title_re=EMB_WINDOW_TITLE, timeout=3)
                all_wins = app_check.windows()
                if all_wins:
                    win = all_wins[0]
                    print(f"   Using first available Wilcom window")
            except Exception as e:
                print(f"❌ Could not connect to Wilcom: {e}")
                return

        # STEP 4: Queue in Wilcom (Shift+Alt+Q)
        if not win:
            print(f"❌ No Wilcom window found, cannot queue")
            return
        
        try:
            # Get window title for logging
            try:
                window_title = win.window_text() or "Unknown"
                print(f"🔗 Connected to Wilcom window: {window_title[:70]}...")
            except:
                print(f"🔗 Connected to Wilcom window")
            
            # IMPORTANT: Activate and focus the window (keyboard shortcuts need active window)
            # Keep it visible for debugging
            try:
                win.restore()  # Make sure it's visible and not minimized
                # Try to move window (may not be available on all window types)
                try:
                    win.move_window(x=100, y=100, width=1200, height=800)  # Position it so we can see it
                except:
                    pass  # move_window not available, that's okay
                win.set_focus()  # Set focus
                time.sleep(1.0)  # Give it time to activate and become visible
                print(f"👁️ Window should now be visible - check if file is loaded")
            except Exception as e:
                print(f"⚠️ Warning: Could not activate window: {e}")
            
            # Force focus multiple times to ensure it's active
            for i in range(3):
                try:
                    win.set_focus()
                    time.sleep(0.3)
            ...
            try:
                if not win.has_focus():
                    print(f"⚠️ Window does not have focus, trying alternative method...")
                    # Try using win32gui to force focus
                    try:
                        import win32gui
                        import win32con
                        hwnd = win.handle
            ...
            # Use multiple methods to send Shift+Alt+Q (more reliable)
            print(f"⌨️ Sending Shift+Alt+Q to queue...")
            print(f"   (Watch the window to see if queue dialog appears)")
            
            # Method 1: Try window-specific type_keys
            try:
                win.type_keys("+!q", set_foreground=True)
                print(f"   ✓ Sent via win.type_keys")
            except Exception as e:
                print(f"   ✗ win.type_keys failed: {e}")
            
            # Method 2: Also try global send_keys as backup
            try:
                from pywinauto.keyboard import send_keys
                send_keys('+%q')  # Shift+Alt+Q
                print(f"   ✓ Sent via global send_keys")
            except Exception as e:
                print(f"   ✗ global send_keys failed: {e}")
            
            # Method 3: Try pyautogui if available
            try:
                import pyautogui
                pyautogui.hotkey("shift", "alt", "q")
                print(f"   ✓ Sent via pyautogui")
            except ImportError:
                pass
            except Exception as e:
                print(f"   ✗ pyautogui failed: {e}")
            
            time.sleep(3.0)  # Give the queue dialog time to appear (longer so you can see it)
            
            # Confirm with Enter - use multiple methods
            print(f"⌨️ Sending Enter to confirm queue...")
            try:
            ...
            time.sleep(2.0)  # Wait for queue to process completely
            
            # Close only the design window (not the main Wilcom app)
            # Use the same method as close_active_document_no_save() which is proven to work
            print(f"🔒 Closing design window with Ctrl+F4...")
            
            # Re-focus the window to ensure it's active
            try:
                win.set_focus()
            ...
            # Use the exact same approach as close_active_document_no_save() - ^{F4}
            close_success = False
            try:
                # Method 1: Use ^{F4} (Ctrl+F4) - exactly as close_active_document_no_save does it
                win.type_keys('^{F4}', set_foreground=True)
                print(f"   ✓ Sent Ctrl+F4 via win.type_keys")
                close_success = True
            except Exception as e:
                print(f"   ✗ win.type_keys failed: {e}, trying global send_keys...")
                # Method 2: Try global send_keys (fallback from close_active_document_no_save)
                try:
                    from pywinauto.keyboard import send_keys
                    send_keys('^{F4}')
                    print(f"   ✓ Sent Ctrl+F4 via global send_keys")
                    close_success = True
                except Exception as e2:
                    print(f"   ✗ global send_keys failed: {e2}")
            
            time.sleep(0.5)  # Wait for close to process
            
            # Handle any "Save changes?" dialog that might appear (same as close_active_document_no_save)
            try:
                ...
                try:
                    save_dlg = desk.window(title_re=r"(Save|Confirm|Close|Wilcom|EmbroideryStudio)", visible_only=True)
                    save_dlg.wait('visible', timeout=2)
                    print(f"   ⚠️ Save dialog detected, clicking 'Don't Save' or 'No'...")
                    # Try to find and click "Don't Save" or "No" button (same patterns as close_active_document_no_save)
                    for pat in [r"(?i)don'?t save", r"(?i)no", r"(?i)discard"]:
                        try:
                            btn = save_dlg.child_window(title_re=pat, control_type="Button")
                            if btn.exists():
                                btn.invoke()
                                print(f"   ✓ Clicked '{pat}' button")
                                break
                        except:
                            continue
                    else:
                        # Fallback: press 'N' for No, then Esc (same as close_active_document_no_save)
                        send_keys('n')
                        time.sleep(0.1)
                        send_keys('{ESC}')
                        print(f"   ✓ Pressed 'N' then Esc")
                except:
                    # No save dialog, that's fine
                    pass
            except:
                pass
            
            if close_success:
                print(f"✅ Sent Ctrl+F4 to close design window (keeps Wilcom app open)")
            else:
                print(f"⚠️ Could not send Ctrl+F4 - design window may still be open")
            
            # Keep window visible for debugging (don't minimize)
            print(f"👁️ Window kept visible for debugging")
            
            print(f"✅ Queued order {order} in Wilcom")
            
        except Exception as e:
            print(f"❌ Error queueing in Wilcom: {e}")
            import traceback
            traceback.print_exc()
        
        # STEP 4: Delete the PDF file from Print Fur PDF folder after processing
        try:
            if os.path.exists(file_path):
                time.sleep(1)  # Brief delay to ensure file isn't locked
                os.remove(file_path)
                print(f"🗑️ Deleted processed PDF: {os.path.basename(file_path)}")
        except Exception as e:
        ...
        print(f"❌ Error in print_fur_pdf_and_queue: {e}")
        import traceback
        traceback.print_exc()

# ================================
# SECTION: PDF Notes Filler (NOTESTEST folder watcher)
# ================================

def fill_pdf_notes_field(pdf_path, notes_text):
    """
    Fill the notes box at the bottom of the PDF page with the provided text.
    Uses PyMuPDF (fitz) to add text overlay to the notes box.
    ...
    try:
        ...
        try:
            ...
            print(f"[PDFNotes] ❌ PyMuPDF (fitz) not available. Cannot add text overlay.")
            return False
        
        # Open the PDF
        doc = fitz.open(pdf_path)
        if len(doc) == 0:
            print(f"[PDFNotes] ❌ PDF has no pages")
            doc.close()
            return False
        
        # Get the first page (where notes box should be)
        page = doc[0]
        page_rect = page.rect
        ...
        # Search for "Notes:" text to find the notes box location
        # The notes box is at the bottom of the page
        notes_box_found = False
        notes_x = None
        notes_y = None
        notes_w = None
        notes_h = None
        
        # Search for "Notes:" text on the page (case-insensitive)
        text_instances = page.search_for("Notes:", flags=fitz.TEXT_DEHYPHENATE)
                ...
                if "lines" in block:
                    for line in block["lines"]:
                        for span in line.get("spans", []):
                            ...
                            if "notes" in text and ":" in text:
                                # Found it - get the position
                                bbox = span.get("bbox", [0, 0, 0, 0])
                                if len(bbox) >= 4:
                                    text_instances = [fitz.Rect(bbox[0], bbox[1], bbox[2], bbox[3])]
                                    break
                        if text_instances:
                            break
                    if text_instances:
        ...
        if text_instances:
            # Found "Notes:" text - the box should be below it
            # Get the bounding box of the first "Notes:" instance
            bbox = text_instances[0]  # Rect object
            notes_x = bbox.x0
            notes_y = bbox.y1 + 5  # Start text just below "Notes:" label (5pt spacing)
            notes_w = page_width - notes_x - 10  # Width from text to right margin
            notes_h = 50  # Height for notes box area
            notes_box_found = True
            print(f"[PDFNotes] Found 'Notes:' text at ({bbox.x0:.1f}, {bbox.y0:.1f})")
        else:
            # If "Notes:" not found, assume notes box is at bottom of page
            # Based on the stamping code: note_y = h - bottom_margin + note_pad
            # bottom_margin is typically 72 points (1 inch), note_pad is 10
            bottom_margin = 72
            note_pad = 10
            # Calculate position: bottom of page minus margin, plus padding, plus label height
            notes_y = page_height - bottom_margin + note_pad + 20  # Add 20 for "Notes:" label height
            notes_x = 10  # Left margin (pad)
            notes_w = page_width - 20  # Full width minus margins
            notes_h = bottom_margin - 2 * note_pad - 20  # Height of notes area
            notes_box_found = True
            print(f"[PDFNotes] 'Notes:' text not found, using default bottom position")
            print(f"[PDFNotes] Page size: {page_width:.1f} x {page_height:.1f}, Notes box at y={notes_y:.1f}")
        
        if not notes_box_found:
            print(f"[PDFNotes] ❌ Could not locate notes box on page")
            doc.close()
            return False
        
        # Clear any existing text in the notes box area (optional - draw white rectangle)
        # Or just add text on top
        
        # Add the notes text
        # Use insert_text to add text at the notes box location
        # Format: insert_text(point, text, fontsize=11, fontname="helv", color=(0,0,0))
        
        # Wrap text to fit within the box width
        font_size = 10
        max_width = notes_w - 10  # Leave some padding
        ...
        for word in words:
            ...
            # Rough estimate: each character is about font_size * 0.6 points wide
        ...
        # If no wrapping needed, use original text
        if not lines:
            lines = [notes_text]
        
        # Use insert_textbox for better text placement (more reliable than insert_text)
        # Create a rectangle for the notes text area
        from fitz import Rect
        
        # Calculate the text box rectangle
        # notes_x, notes_y is where we want to start (below "Notes:" label)
        text_x0 = notes_x + 5  # Left padding
        text_x1 = notes_x + notes_w - 5  # Right padding
        text_y0 = notes_y  # Top of text area
        text_y1 = notes_y + notes_h  # Bottom of text area (don't exceed box)
        
        # Make sure we don't go past page boundaries
        if text_y1 > page_height - 10:
            text_y1 = page_height - 10
        
        text_rect = Rect(text_x0, text_y0, text_x1, text_y1)
        
        print(f"[PDFNotes] Text box rectangle: ({text_x0:.1f}, {text_y0:.1f}) to ({text_x1:.1f}, {text_y1:.1f})")
        print(f"[PDFNotes] Text to insert: {notes_text[:100]}...")
        
        # Combine all lines into a single text block with newlines
        full_text = "\n".join(lines)
        
        try:
            # Use insert_textbox which handles wrapping automatically
            # This is more reliable than insert_text for placing text in a specific area
            rc = page.insert_textbox(
                text_rect,
                full_text,
                fontsize=font_size,
                fontname="helv",  # Arial equivalent
                color=(0, 0, 0),  # Black
                align=0  # Left align (0=left, 1=center, 2=right)
            )
            
            if rc < 0:
                print(f"[PDFNotes] ⚠️ Warning: insert_textbox returned {rc} (text may not fit)")
                print(f"[PDFNotes] Trying to insert text line by line...")
                
                # Fallback: insert line by line
                line_height = font_size * 1.3
                current_y = notes_y
                for i, line in enumerate(lines):
                    if current_y > text_y1 - font_size:
                        break
                    line_rect = Rect(text_x0, current_y - font_size, text_x1, current_y)
                    try:
                        page.insert_textbox(line_rect, line, fontsize=font_size, color=(0, 0, 0), align=0)
                    except Exception as e:
                        print(f"[PDFNotes] Could not insert line {i}: {e}")
                    current_y += line_height
            else:
                print(f"[PDFNotes] ✅ Text inserted successfully (return code: {rc})")
                
        except Exception as e:
            print(f"[PDFNotes] ❌ Error inserting text: {e}")
            import traceback
            traceback.print_exc()
            # Try one more fallback with insert_text
            try:
                print(f"[PDFNotes] Trying fallback method: insert_text...")
                for i, line in enumerate(lines):
                    y_pos = notes_y + (i * font_size * 1.3)
                    if y_pos > text_y1:
                        break
                    page.insert_text(
                        (text_x0, y_pos),
                ...
                print(f"[PDFNotes] ✅ Fallback method succeeded")
            except Exception as e2:
                print(f"[PDFNotes] ❌ Fallback method also failed: {e2}")
                raise
        
        # Save the updated PDF
        temp_path = pdf_path + ".tmp"
        print(f"[PDFNotes] Saving updated PDF to: {temp_path}")
        doc.save(temp_path)
        doc.close()
        
        # Replace original with updated version
        import shutil
        print(f"[PDFNotes] Replacing original PDF with updated version...")
        shutil.move(temp_path, pdf_path)
        print(f"[PDFNotes] ✅ PDF file updated successfully")
        
        print(f"[PDFNotes] ✅ Successfully added notes text to PDF ({len(lines)} lines)")
        return True
        
    except Exception as e:
        print(f"[PDFNotes] ❌ Error filling PDF notes field: {e}")
        import traceback
        traceback.print_exc()
        return False


def fill_pdf_notes_from_sheets(pdf_path):
    """
    Extract order number from PDF filename, get notes from Google Sheets,
    and fill the PDF's notes field.
    """
    try:
        # Extract order number from filename
        # Expected format: something like "266_266_Stamped.pdf" or "266.pdf"
        filename = os.path.basename(pdf_path)
        filename_no_ext = os.path.splitext(filename)[0]
        
        # Try to extract order number (first number sequence)
        import re
        match = re.search(r'(\d+)', filename_no_ext)
        if not match:
            print(f"[PDFNotes] ❌ Could not extract order number from filename: {filename}")
            return False
        
        order_number = match.group(1)
        print(f"[PDFNotes] 📋 Extracted order number: {order_number}")
        
        # Get spreadsheet connection
        try:
            worksheet, spreadsheet = connect_google_sheet("JR and Co.", "Production Orders")
        except Exception as e:
            print(f"[PDFNotes] ❌ Could not connect to Google Sheets: {e}")
            return False
        
        # Get notes from Google Sheets
        notes = get_notes(order_number, spreadsheet)
        if not notes:
            print(f"[PDFNotes] ⚠️ No notes found for order {order_number} in Google Sheets")
            print(f"[PDFNotes]   (This is OK - continuing with print/queue anyway)")
            return False
        
        print(f"[PDFNotes] 📝 Found notes for order {order_number}: {notes[:100]}...")
        print(f"[PDFNotes]   Full notes length: {len(notes)} characters")
        
        # Fill the PDF notes field
        print(f"[PDFNotes] Attempting to fill notes in PDF: {pdf_path}")
        success = fill_pdf_notes_field(pdf_path, notes)
        if success:
            print(f"[PDFNotes] ✅ Successfully filled notes for order {order_number}")
        else:
            print(f"[PDFNotes] ⚠️ Could not fill notes field in PDF")
            print(f"[PDFNotes]   (This is OK - continuing with print/queue anyway)")
        
        return success
        
    except Exception as e:
        print(f"[PDFNotes] ❌ Error in fill_pdf_notes_from_sheets: {e}")
        import traceback
        traceback.print_exc()
        return False
...
notes_processed_files = set()
last_notes_file_time = {}

class NotesPDFHandler(FileSystemEventHandler):
    """File system event handler for NOTESTEST folder."""
    def on_created(self, event):
        if event.is_directory:
            return
        ...
        # Debounce (4 seconds)
        now = time.time()
        last = last_notes_file_time.get(file_path, 0)
        if now - last < 4:
            return
        last_notes_file_time[file_path] = now

        # Avoid reprocessing
        if file_path in notes_processed_files:
            return
        ...
        print("================================")
        print("📝 PDF NOTES FILLER")
        print("================================")
        print(f"Incoming file: {file_path}")

        # Process the PDF
        success = fill_pdf_notes_from_sheets(file_path)
        
        if success:
            notes_processed_files.add(file_path)
            print(f"✅ Successfully processed: {os.path.basename(file_path)}")
        else:
            print(f"⚠️ Could not fill notes (may not have form fields or no notes found)")
        
        print("================================\n")


def start_notes_pdf_watcher():
    """Start watching the NOTESTEST folder for new PDFs to fill notes."""
    NOTESTEST_FOLDER = r"G:\My Drive\NOTESTEST"
    
    if not os.path.exists(NOTESTEST_FOLDER):
        print(f"[PDFNotes] ⚠️ Folder does not exist: {NOTESTEST_FOLDER}")
        print(f"[PDFNotes] Creating folder...")
        try:
            os.makedirs(NOTESTEST_FOLDER, exist_ok=True)
            print(f"[PDFNotes] ✅ Created folder: {NOTESTEST_FOLDER}")
        except Exception as e:
            print(f"[PDFNotes] ❌ Could not create folder: {e}")
            return
    
    print(f"👀 Watching NOTESTEST folder: {NOTESTEST_FOLDER}")
    
    event_handler = NotesPDFHandler()
    observer = Observer()
    observer.schedule(event_handler, NOTESTEST_FOLDER, recursive=False)
    observer.start()
    
    print(f"[PDFNotes] ✅ Notes PDF watcher started")
...
class PrintFurHandler(FileSystemEventHandler):
    """File system event handler for Print Fur PDF folder."""
    def on_created(self, event):
        if event.is_directory:
            return
        ...
        # Debounce (4 seconds)
        now = time.time()
        last = last_fur_print_time.get(file_path, 0)
        if now - last < 4:
            return
        last_fur_print_time[file_path] = now

        # Make sure file is fully written
        time.sleep(2)

        print_fur_pdf_and_queue(file_path)

def start_print_fur_watcher():
    """Start watching the Print Fur PDF folder for new PDFs."""
    print(f"👀 Watching Print Fur PDF folder: {PRINT_FUR_FOLDER}")
    print(f"🖨️ Will print to default printer and queue in Wilcom")

    if not os.path.exists(PRINT_FUR_FOLDER):
        print(f"⚠️ Warning: Print Fur PDF folder does not exist: {PRINT_FUR_FOLDER}")
        print(f"   Creating folder...")
        try:
            os.makedirs(PRINT_FUR_FOLDER, exist_ok=True)
            print(f"✅ Created folder: {PRINT_FUR_FOLDER}")
        except Exception as e:
            print(f"❌ Could not create folder: {e}")
            return

    event_handler = PrintFurHandler()
    observer = Observer()
    observer.schedule(event_handler, PRINT_FUR_FOLDER, recursive=False)
    observer.start()
...
def open_url_in_browser(url: str, force_activate: bool = False):
    ...
    print(f"[open_url_in_browser] Called with URL: {url}", flush=True)

    try:
        # --- Dedup ---
        now = time.time()
        # For success pages, use more aggressive deduplication (check base URL)
        is_success_page = "success" in url.lower() or "127.0.0.1:5001/success" in url
        print(f"[open_url_in_browser] Is success page: {is_success_page}", flush=True)
        dedup_key = url
        if is_success_page:
            # For success pages, deduplicate based on the route, not full URL
            # This prevents multiple windows if called multiple times with different params
            dedup_key = "http://127.0.0.1:5001/success"
        
        last = _last_opened_at.get(dedup_key, 0.0)
        dedup_window = DEDUP_WINDOW_SEC * 2 if is_success_page else DEDUP_WINDOW_SEC
        if now - last < dedup_window:
            print(f"[open_url] Deduplicated: {dedup_key} (last opened {now - last:.1f}s ago)", flush=True)
            return
        _last_opened_at[dedup_key] = now

        # --- Remember currently focused window ---
        prev_hwnd = win32gui.GetForegroundWindow()

        # Detect secondary monitor coordinates before launching Chrome
        is_success_page_pre = "success" in url.lower() or "127.0.0.1:5001/success" in url
        secondary_x_pre = SECONDARY_X
        secondary_y_pre = SECONDARY_Y
        secondary_w_pre = SECONDARY_W
        secondary_h_pre = SECONDARY_H
        
        # If coordinates not set, detect now
        if (secondary_w_pre == 0 or secondary_h_pre == 0) and is_success_page_pre:
            try:
                from screeninfo import get_monitors
                mons = get_monitors()
                secondary = next((m for m in mons if not m.is_primary), None)
                if secondary:
                    secondary_x_pre = secondary.x
                    secondary_y_pre = secondary.y
                    secondary_w_pre = secondary.width
                    secondary_h_pre = secondary.height
                    print(f"[open_url] Pre-detected secondary monitor: ({secondary_x_pre},{secondary_y_pre},{secondary_w_pre}x{secondary_h_pre})", flush=True)
            except Exception:
                pass
        ...
        # For success pages, try to set initial window position and size
        if is_success_page_pre and secondary_w_pre > 0 and secondary_h_pre > 0:
            chrome_args.extend([
                f"--window-position={secondary_x_pre},{secondary_y_pre}",
                f"--window-size={secondary_w_pre},{secondary_h_pre}",
            ])
            print(f"[open_url] Setting Chrome initial position: ({secondary_x_pre},{secondary_y_pre}) size: ({secondary_w_pre},{secondary_h_pre})", flush=True)
        ...
        # Track which window we want to keep (will be set when we find it)
        target_hwnd = [None]  # Use list so we can modify from nested functions

        # --- Move + RESIZE ONLY the new Chrome window ---
        def _move_only_new_chrome():
            # Start closing Chrome windows on primary monitor immediately
            if is_success_page_pre:
                def aggressive_close_primary():
                    """Aggressively close Chrome windows on primary monitor as they appear"""
                    for i in range(20):  # Run for up to 10 seconds
                        def close_aggressive(hwnd, _):
                            try:
                                if not win32gui.IsWindowVisible(hwnd):
                                    return
                                
                                class_name = win32gui.GetClassName(hwnd)
                                # CRITICAL: ONLY Chrome browser windows
                                if class_name != "Chrome_WidgetWin_1":
                                    return  # Not Chrome - don't touch!
                                
                                # Verify it's actually Chrome process
                                try:
                                    _, pid_ag = win32process.GetWindowThreadProcessId(hwnd)
                                    try:
                                        import psutil
                                        try:
                                            proc_ag = psutil.Process(pid_ag)
                                            if "chrome" not in proc_ag.name().lower():
                                                return  # Not Chrome process - don't close!
                                        except (psutil.NoSuchProcess, psutil.AccessDenied):
                                            # Can't verify - but if title matches, assume it's Chrome
                                            title_ag2 = win32gui.GetWindowText(hwnd)
                                            if not title_ag2 or ("success" not in title_ag2.lower() and "127.0.0.1" not in title_ag2.lower()):
                                                return  # Doesn't look like our Chrome window
                                    except ImportError:
                                        # psutil not available - check title as fallback
                                        title_ag2 = win32gui.GetWindowText(hwnd)
                                        if not title_ag2 or ("success" not in title_ag2.lower() and "127.0.0.1" not in title_ag2.lower()):
                                            return  # Doesn't look like our Chrome window
                                except Exception:
                                    return  # Can't verify - don't risk closing it!
                                
                                # Don't close our target window
                                if target_hwnd[0] and hwnd == target_hwnd[0]:
                                    return
                                
                                rect_ag = win32gui.GetWindowRect(hwnd)
                                # On primary monitor
                                if rect_ag[0] >= -100 and rect_ag[1] >= -100:
                                    title_ag = win32gui.GetWindowText(hwnd)
                                    if title_ag and ("success" in title_ag.lower() or "127.0.0.1" in title_ag.lower()):
                                        print(f"[open_url] Aggressively closing Chrome window on primary: HWND={hwnd}, Title='{title_ag[:40]}', Pos=({rect_ag[0]},{rect_ag[1]})", flush=True)
                                        win32gui.PostMessage(hwnd, win32con.WM_CLOSE, 0, 0)
                            except Exception as e:
                                print(f"[open_url] Error in aggressive close: {e}", flush=True)
                        win32gui.EnumWindows(close_aggressive, None)
                        time.sleep(0.5)  # Check every 0.5 seconds
                threading.Thread(target=aggressive_close_primary, daemon=True).start()
            
            time.sleep(1.5)  # allow window to be created and fully loaded
            
            is_success_page = "success" in url.lower() or "127.0.0.1:5001/success" in url
            
            # Always detect monitors fresh to ensure we have correct coordinates
            target_x = 0
            target_y = 0
            target_w = 0
            target_h = 0
            ...
            try:
                from screeninfo import get_monitors
                mons = get_monitors()
                print(f"[open_url] Detected {len(mons)} monitor(s)", flush=True)
                
                # Find secondary monitor (non-primary)
                secondary = next((m for m in mons if not m.is_primary), None)
                if secondary:
                    target_x = secondary.x
                    target_y = secondary.y
                    target_w = secondary.width
                    target_h = secondary.height
                    print(f"[open_url] Using SECONDARY monitor: ({target_x},{target_y},{target_w}x{target_h})", flush=True)
                else:
                    # Fallback to primary monitor if no secondary
                    primary = next((m for m in mons if m.is_primary), None)
                    if primary:
                        target_x = primary.x
                        target_y = primary.y
                        target_w = primary.width
                        target_h = primary.height
                        print(f"[open_url] No secondary monitor found, using PRIMARY: ({target_x},{target_y},{target_w}x{target_h})", flush=True)
                    else:
                        print(f"[open_url] ERROR: No monitors detected!", flush=True)
                        return
            except Exception as e:
                print(f"[open_url] Monitor detection error: {e}", flush=True)
                import traceback
                traceback.print_exc()
                return
            
            if target_w == 0 or target_h == 0:
                print(f"[open_url] ERROR: Invalid monitor dimensions!", flush=True)
                return
            
            found_window = False
            processed_windows = []
            all_chrome_windows = []  # For debugging
            launch_time = time.time()  # Track when we launched Chrome
            ...
            def enum_cb(hwnd, _):
                ...
                try:
                    ...
                    # Check if this is a Chrome browser window (not a child window)
                    class_name = win32gui.GetClassName(hwnd)
                    if class_name != "Chrome_WidgetWin_1":
                        return
                    ...
                    # For debugging: collect all Chrome windows
                    all_chrome_windows.append((hwnd, pid, window_text))
                    
                    # Check if window has a title (main browser window)
                    if not window_text or window_text.strip() == "":
                        return
                    
                    # For success pages, be very lenient - find any Chrome window with "Success" in title
                    # or any Chrome window that appeared recently (within last 5 seconds)
                    is_likely_our_window = False
                    if is_success_page:
                        title_lower = window_text.lower()
                        # Check if title contains "Success", "127.0.0.1", or "localhost"
                        if "success" in title_lower or "127.0.0.1" in title_lower or "localhost" in title_lower:
                            is_likely_our_window = True
                            print(f"[open_url] Matched by title: '{window_text}'", flush=True)
                        # Also check if this is a recently created window (Chrome child process)
                        # We'll be lenient and accept any Chrome window if we can't find by title
                        # But prefer windows with matching titles
                    else:
                        # For non-success pages, try to match by PID
                        if pid == chrome_pid:
                            is_likely_our_window = True
                    
                    if not is_likely_our_window:
                        return
                    
                    # Only process the first matching window
                    if hwnd in processed_windows:
                        return
                    
                    processed_windows.append(hwnd)
                    target_hwnd[0] = hwnd  # Mark this as our target window
                    found_window = True
                    print(f"[open_url] Found matching Chrome window: PID={pid}, Title='{window_text}'", flush=True)
                    
                    # For success pages, immediately close any other Chrome windows on primary monitor
                    if is_success_page:
                        def close_primary_monitor_windows():
                            time.sleep(0.1)  # Very brief delay
                            def close_cb(hwnd2, _):
                                try:
                                    if hwnd2 == hwnd:  # Don't close our target window
                                        return
                                    if not win32gui.IsWindowVisible(hwnd2):
                                        return
                                    
                                    class_name2 = win32gui.GetClassName(hwnd2)
                                    # CRITICAL: Only close Chrome browser windows
                                    if class_name2 != "Chrome_WidgetWin_1":
                                        return
                                    
                                    # Verify it's actually Chrome (with fallback if psutil not available)
                                    try:
                                        ...
                                        try:
                                            ...
                                            try:
                                                ...
                                                if "chrome" not in proc2.name().lower():
                                                    return  # Not Chrome - don't close!
                                            except (psutil.NoSuchProcess, psutil.AccessDenied):
                                                # Can't verify - but if title matches, assume it's Chrome
                                                title_check2 = win32gui.GetWindowText(hwnd2)
                                                if not title_check2 or ("success" not in title_check2.lower() and "127.0.0.1" not in title_check2.lower()):
                                                    return  # Doesn't look like our Chrome window
                                        except ImportError:
                                            # psutil not available - check title as fallback
                                            title_check2 = win32gui.GetWindowText(hwnd2)
                                        ...
                                        return  # Can't verify - don't risk closing!
                                    
                                    rect_check = win32gui.GetWindowRect(hwnd2)
                                    # Check if window is on primary monitor (positive or near-zero coordinates)
                                    if rect_check[0] >= -100 and rect_check[1] >= -100:
                                        title_check = win32gui.GetWindowText(hwnd2)
                                        if title_check and ("success" in title_check.lower() or "127.0.0.1" in title_check.lower()):
                                            print(f"[open_url] Closing Chrome window on primary monitor: HWND={hwnd2}, Title='{title_check[:50]}', Pos=({rect_check[0]},{rect_check[1]})", flush=True)
                                            win32gui.PostMessage(hwnd2, win32con.WM_CLOSE, 0, 0)
                                except Exception as e:
                                    print(f"[open_url] Error in immediate close: {e}", flush=True)
                            win32gui.EnumWindows(close_cb, None)
                        threading.Thread(target=close_primary_monitor_windows, daemon=True).start()
                    
                    if is_success_page:
                        # For success pages: maximize to fullscreen on target monitor
                        print(f"[open_url] Found success window, moving to monitor at ({target_x},{target_y})", flush=True)
                        
                        # First, restore window in case it's minimized
                        win32gui.ShowWindow(hwnd, win32con.SW_RESTORE)
                        time.sleep(0.3)
                        
                        # Get current window position to check if it's already on the right monitor
                        rect = win32gui.GetWindowRect(hwnd)
                        current_x = rect[0]
                        current_y = rect[1]
                        print(f"[open_url] Current window position: ({current_x}, {current_y})", flush=True)
                        
                        # Force move to target monitor - use SWP_NOZORDER to keep it behind
                        ...
                        print(f"[open_url] Window position after move: ({rect2[0]}, {rect2[1]})", flush=True)
                        
                        # Then maximize - this should fill the entire monitor
                        win32gui.ShowWindow(hwnd, win32con.SW_MAXIMIZE)
                        time.sleep(0.3)
                        
                        # Verify final position
                        rect3 = win32gui.GetWindowRect(hwnd)
                        final_w = rect3[2] - rect3[0]
                        final_h = rect3[3] - rect3[1]
                        print(f"[open_url] Final window size: {final_w}x{final_h} at ({rect3[0]}, {rect3[1]})", flush=True)
                        print(f"[open_url] Target monitor size: {target_w}x{target_h}", flush=True)
                        
                        print(f"[open_url] Success window should now be maximized on monitor at ({target_x},{target_y})", flush=True)
                        
                        # After positioning, aggressively close ALL Chrome windows on primary monitor
                        def close_all_primary_chrome():
                            time.sleep(0.5)  # Brief delay
                            closed_count = 0
                            def close_all_cb(hwnd_all, _):
                                nonlocal closed_count
                                try:
                                    if not win32gui.IsWindowVisible(hwnd_all):
                                        return
                                    if hwnd_all == hwnd:  # Don't close our target
                                        return
                                    
                                    class_name_all = win32gui.GetClassName(hwnd_all)
                                    if class_name_all != "Chrome_WidgetWin_1":
                                        return
                                    
                                    title_all = win32gui.GetWindowText(hwnd_all)
                                    if not title_all or "success" not in title_all.lower():
                                        return
                                    
                                    rect_all = win32gui.GetWindowRect(hwnd_all)
                                    # If on primary monitor (not our target secondary)
                                    if rect_all[0] >= -100 and rect_all[1] >= -100:
                                        print(f"[open_url] Closing ALL Chrome window on primary: HWND={hwnd_all}, Title='{title_all[:50]}', Pos=({rect_all[0]},{rect_all[1]})", flush=True)
                                        win32gui.PostMessage(hwnd_all, win32con.WM_CLOSE, 0, 0)
                                        closed_count += 1
                                        time.sleep(0.1)
                                except Exception:
                                    pass
                            
                            # Run multiple times to catch windows that appear
                            for i in range(5):
                                win32gui.EnumWindows(close_all_cb, None)
                                if i < 4:
                                    time.sleep(0.3)
                            
                            if closed_count > 0:
                                print(f"[open_url] Closed {closed_count} Chrome window(s) on primary monitor", flush=True)
                        threading.Thread(target=close_all_primary_chrome, daemon=True).start()
                    
                    else:
                        # For other pages: restore and position normally
                        win32gui.ShowWindow(hwnd, win32con.SW_RESTORE)
                        time.sleep(0.1)

                        # HARD clamp to target monitor bounds
                        win32gui.SetWindowPos(
                            hwnd,
                            win32con.HWND_TOP,
                            target_x,
                    ...
                    print(f"[open_url] Window positioning error: {e}", flush=True)

            # Try multiple times in case window isn't ready yet
            print(f"[open_url] Searching for Chrome window (PID {chrome_pid})...", flush=True)
            
            # Log all visible windows on first attempt for debugging
            if is_success_page:
                def log_all_windows():
                    def log_cb(hwnd_log, _):
                        try:
                            if not win32gui.IsWindowVisible(hwnd_log):
                                return
                            class_log = win32gui.GetClassName(hwnd_log)
                            title_log = win32gui.GetWindowText(hwnd_log)
                            if title_log and ("success" in title_log.lower() or "127.0.0.1" in title_log.lower() or "create" in title_log.lower()):
                                rect_log = win32gui.GetWindowRect(hwnd_log)
                                try:
                                    _, pid_log = win32process.GetWindowThreadProcessId(hwnd_log)
                                    import psutil
                                    try:
                                        proc_log = psutil.Process(pid_log)
                                        proc_name_log = proc_log.name()
                                    except:
                                        proc_name_log = "unknown"
                                except:
                                    proc_name_log = "unknown"
                                print(f"[open_url] WINDOW LOG: HWND={hwnd_log}, Class='{class_log}', PID={pid_log if 'pid_log' in locals() else 'N/A'}, Process='{proc_name_log}', Title='{title_log[:60]}', Pos=({rect_log[0]},{rect_log[1]}), Size=({rect_log[2]-rect_log[0]}x{rect_log[3]-rect_log[1]})", flush=True)
                        except Exception:
                            pass
                    print(f"[open_url] === LOGGING ALL WINDOWS WITH SUCCESS/CREATE IN TITLE ===", flush=True)
                    win32gui.EnumWindows(log_cb, None)
                    print(f"[open_url] === END WINDOW LOG ===", flush=True)
                threading.Thread(target=log_all_windows, daemon=True).start()
            
            for attempt in range(20):  # Even more attempts
                all_chrome_windows.clear()
                
                # Before searching, close any Chrome windows on primary monitor (for success pages)
                if is_success_page and attempt > 0:
                    def close_primary_now():
                        def close_cb_now(hwnd2, _):
                            try:
                                if not win32gui.IsWindowVisible(hwnd2):
                                    return
                                
                                class_name_now = win32gui.GetClassName(hwnd2)
                                # CRITICAL: Only close Chrome browser windows
                                if class_name_now != "Chrome_WidgetWin_1":
                                    return
                                
                                # Verify it's actually Chrome (with fallback if psutil not available)
                                try:
                                    _, pid_now = win32process.GetWindowThreadProcessId(hwnd2)
                                    try:
                                        import psutil
                                        try:
                                            proc_now = psutil.Process(pid_now)
                                            if "chrome" not in proc_now.name().lower():
                                                return  # Not Chrome - don't close!
                                        except (psutil.NoSuchProcess, psutil.AccessDenied):
                                            # Can't verify - but if title matches, assume it's Chrome
                                            title_now2 = win32gui.GetWindowText(hwnd2)
                                            if not title_now2 or ("success" not in title_now2.lower() and "127.0.0.1" not in title_now2.lower()):
                                                return  # Doesn't look like our Chrome window
                                    except ImportError:
                                        # psutil not available - check title as fallback
                                        title_now2 = win32gui.GetWindowText(hwnd2)
                                ...
                                rect_now = win32gui.GetWindowRect(hwnd2)
                                # Check if window is on primary monitor
                                if rect_now[0] >= -100 and rect_now[1] >= -100:
                                    title_now = win32gui.GetWindowText(hwnd2)
                                    if title_now and ("success" in title_now.lower() or "127.0.0.1" in title_now.lower()):
                                        # Only close if it's not our target window
                                        if hwnd2 not in processed_windows:
                                            print(f"[open_url] Preemptively closing Chrome window on primary monitor: HWND={hwnd2}, Pos=({rect_now[0]},{rect_now[1]})", flush=True)
                                            win32gui.PostMessage(hwnd2, win32con.WM_CLOSE, 0, 0)
                            except Exception as e:
                                print(f"[open_url] Error in preemptive close: {e}", flush=True)
                        win32gui.EnumWindows(close_cb_now, None)
                    close_primary_now()
                
                win32gui.EnumWindows(enum_cb, None)
                if found_window:
                    print(f"[open_url] Found and positioned window on attempt {attempt + 1}", flush=True)
                    break
                if attempt < 19:
                    if attempt == 0 or attempt % 4 == 0:
                        print(f"[open_url] Attempt {attempt + 1}: Found {len(all_chrome_windows)} Chrome window(s)", flush=True)
                        for h, p, t in all_chrome_windows[:5]:  # Show first 5
                            print(f"  - HWND={h}, PID={p}, Title='{t[:60]}'", flush=True)
                    time.sleep(0.6)  # Longer delay
            
            if not found_window:
                print(f"[open_url] WARNING: Could not find Chrome window after 20 attempts!", flush=True)
                print(f"[open_url] Found {len(all_chrome_windows)} Chrome window(s) total:", flush=True)
                for h, p, t in all_chrome_windows:
                    print(f"  - HWND={h}, PID={p}, Title='{t[:80]}'", flush=True)
                
                # Last resort: find the most recently created Chrome window with a title
                if is_success_page and all_chrome_windows:
                    print(f"[open_url] Last resort: Using most recently found Chrome window...", flush=True)
                    # Use the last window we found (most likely to be ours)
                    hwnd, pid, title = all_chrome_windows[-1]
                    found_window = True
                    processed_windows.append(hwnd)
                    print(f"[open_url] Using window: HWND={hwnd}, PID={pid}, Title='{title}'", flush=True)
                    
                    # Manually run positioning code since we found it in fallback
                    try:
                        print(f"[open_url] Found success window (fallback), moving to monitor at ({target_x},{target_y})", flush=True)
                        ...
                        print(f"[open_url] Error positioning fallback window: {e}", flush=True)
                        import traceback
                        traceback.print_exc()
            
            # Close any extra Chrome windows that might have opened
            if found_window and is_success_page:
                def close_extra_windows():
                    time.sleep(0.5)  # Brief delay to let main window settle
                    
                    # List of window class names to NEVER close (whitelist)
                    SAFE_CLASSES = [
                        "Chrome_WidgetWin_0",  # Chrome UI elements (not browser windows)
                        "Chrome_RenderWidgetHostHWND",  # Chrome renderer
                        "VSCodeWindowClass",  # VS Code
                        "Chrome_WidgetWin_1",  # We'll check this more carefully
                    ]
                    ...
                    def close_cb(hwnd, _):
                        try:
                            ...
                            # CRITICAL: Only close Chrome_WidgetWin_1 windows, and verify it's actually Chrome
                            if class_name != "Chrome_WidgetWin_1":
                                # Log non-Chrome windows for debugging
                                if window_text and ("success" in window_text.lower() or "127.0.0.1" in window_text.lower()):
                                    rect_log = win32gui.GetWindowRect(hwnd)
                                    print(f"[open_url] DEBUG: Non-Chrome window with success-related title: HWND={hwnd}, Class='{class_name}', Title='{window_text[:50]}', Pos=({rect_log[0]},{rect_log[1]})", flush=True)
                                return  # Not a Chrome browser window - don't touch it!
                            
                            # Double-check: verify it's actually Chrome by checking process name
                            # Try psutil first, but if not available, use a safer fallback
                            is_chrome = False
                            try:
                                _, pid = win32process.GetWindowThreadProcessId(hwnd)
                                ...
                                try:
                                    ...
                                    try:
                                        ...
                                        if "chrome" in proc_name:
                                            ...
                                            print(f"[open_url] DEBUG: Window has Chrome class but process is '{proc_name}': HWND={hwnd}, Title='{window_text[:50] if window_text else 'N/A'}'", flush=True)
                                            return  # Not Chrome - don't close!
                                    except (psutil.NoSuchProcess, psutil.AccessDenied):
                                        # Can't verify process - for Chrome_WidgetWin_1, assume it's Chrome
                                        # but only if title suggests it's a browser window
                                        if window_text and ("success" in window_text.lower() or "127.0.0.1" in window_text.lower() or "google chrome" in window_text.lower()):
                                            is_chrome = True
                                        else:
                                            return  # Can't verify and doesn't look like our Chrome window
                                except ImportError:
                                    # psutil not available - use fallback: check if PID matches our Chrome PID or is close to it
                                    # Chrome spawns child processes, so check if PID is in a reasonable range
                                    # For now, if class is Chrome_WidgetWin_1 and title matches, assume it's Chrome
                                    if window_text and ("success" in window_text.lower() or "127.0.0.1" in window_text.lower() or "google chrome" in window_text.lower()):
                                        is_chrome = True
                                    else:
                                        return  # Doesn't look like our Chrome window
                            except Exception as e:
                                # If we can't get PID, be conservative
                                return
                            
                            if not is_chrome:
                                return
                            
                            # Get window position to check if it's on the wrong monitor
                            rect = win32gui.GetWindowRect(hwnd)
                            window_x = rect[0]
                            window_y = rect[1]
                            
                            # Check if this window is on the primary monitor (not our target secondary)
                            is_on_primary = (window_x >= -100 and window_y >= -100)
                            
                            # Close windows that:
                            # 1. Are not in our processed list (not the one we positioned)
                            # 2. Are on the primary monitor (wrong monitor)
                            # 3. Have "Success" in title but are not our target window
                            should_close = False
                            if hwnd not in processed_windows:
                                if is_on_primary:
                                    should_close = True
                                    print(f"[open_url] Closing Chrome window on primary monitor: HWND={hwnd}, Title='{window_text[:50] if window_text else 'N/A'}', Pos=({window_x},{window_y})", flush=True)
                                elif window_text and "success" in window_text.lower():
                                    # Another success window that's not ours
                                    should_close = True
                                    print(f"[open_url] Closing duplicate Chrome success window: HWND={hwnd}, Title='{window_text[:50]}', Pos=({window_x},{window_y})", flush=True)
                            
                            if should_close:
                                win32gui.PostMessage(hwnd, win32con.WM_CLOSE, 0, 0)
                                time.sleep(0.05)  # Small delay between closes
                        except Exception as e:
                            print(f"[open_url] Error closing extra window: {e}", flush=True)
                            import traceback
                            traceback.print_exc()
                    
                    # Try multiple times to catch windows that open later
                    for attempt in range(5):
                        win32gui.EnumWindows(close_cb, None)
                        if attempt < 4:
                            time.sleep(0.3)
                threading.Thread(target=close_extra_windows, daemon=True).start()
...
def _open_success_page(order: str, dept: str | None = None):
    ...
    print(f"[_open_success_page] Opening success page for order {order}, dept {dept}", flush=True)
...
def _handle_scanned_url(url: str):
    ...
    if action == "SHIP":
        # Shipping flows: do NOT open the scanned Apps Script URL in a tab.
    ...
    # SHIPPING: headless GET already done; show green page for 7s
    if action == "SHIP":
...
if __name__ == "__main__":
    ...
    flask_thread.start()
    print("[Startup] Embedded Flask server started on http://127.0.0.1:5001 (localhost only)")
    print("[Startup] Print server endpoints available: /print, /queue-emb")
    ...
    # Create a network-accessible print server (listens on all interfaces)
    # This allows other computers on the network to print
    def create_network_print_server():
        """Create a Flask app for network-accessible print server."""
        network_app = Flask(__name__)
        
        # Allow CORS from any origin for network access
        CORS(network_app, resources={
            r"/*": {
        ...
        @network_app.route("/print", methods=["OPTIONS", "POST"])
        def network_print():
            """Handle print requests from network."""
            try:
        ...
        @network_app.route("/queue-emb", methods=["POST"])
        def network_queue_emb():
            """Handle queue-emb requests from network."""
            from pywinauto.keyboard import send_keys
    ...
    try:
        network_print_app = create_network_print_server()
        network_print_thread = threading.Thread(
            target=lambda: network_print_app.run(
        ...
        network_print_thread.start()
        print(f"[Startup] Network print server started on http://{local_ip}:5009 (accessible from network)")
        print(f"[Startup] Network print server also available on http://0.0.0.0:5009")
        print(f"[Startup] Other computers can use: http://{local_ip}:5009/print")
    except Exception as e:
        print(f"[Startup] Warning: Could not start network print server on port 5009: {e}")
        print("[Startup] Print server still available on localhost port 5001")
    
    # Start UPS label watcher in background thread
    try:
        label_watcher_thread = threading.Thread(target=start_label_watcher, daemon=True)
        label_watcher_thread.start()
        print("[Startup] UPS Label watcher started")
    except Exception as e:
        print(f"[Startup] Warning: Could not start label watcher: {e}")
    
    # Start Print Fur PDF watcher in background thread
    try:
        print_fur_watcher_thread = threading.Thread(target=start_print_fur_watcher, daemon=True)
        print_fur_watcher_thread.start()
        print("[Startup] Print Fur PDF watcher started")
    except Exception as e:
        print(f"[Startup] Warning: Could not start Print Fur PDF watcher: {e}")
    
    # Start Notes PDF watcher in background thread
    try:
        notes_pdf_watcher_thread = threading.Thread(target=start_notes_pdf_watcher, daemon=True)
        notes_pdf_watcher_thread.start()
        print("[Startup] Notes PDF watcher started (NOTESTEST folder)")
    except Exception as e:
        print(f"[Startup] Warning: Could not start Notes PDF watcher: {e}")
    ...
    # 6.5) Start status overlay (green checkmark on secondary monitor)
    try:
        overlay_thread = threading.Thread(target=run_status_overlay, daemon=True)
        overlay_thread.start()
        print("[Startup] Status overlay thread started (green checkmark on secondary monitor)")
        time.sleep(0.5)  # Give it a moment to initialize
    except Exception as e:
        print(f"[Startup] Status overlay failed: {e}")
        import traceback
        traceback.print_exc()
