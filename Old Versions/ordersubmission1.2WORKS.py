...
class OrderEntryApp(tk.Frame):
    ...
    def upload_prod_file(self):
        ...
        if new_filepaths:
            self.prod_file_paths.extend(list(new_filepaths))
    ...
    def upload_print_files(self):
        ...
        if new_filepaths:
            self.print_file_paths.extend(list(new_filepaths))
    ...
    def process_new_materials(self):
        ...
        for field_name, var, widget in material_fields:
            ...
            if mat and mat not in union_list:
                ...
                if response:
                    dialog = NewMaterialDialog(tk._get_default_root(), mat)
                    if not dialog.min_inv or not dialog.reorder or not dialog.price or not dialog.unit:
                        widget.configure(background="yellow")
                        raise Exception(f"Missing info for {field_name}.")
                    else:
                        create_new_material_inventory(mat, dialog.unit, dialog.min_inv, dialog.reorder, dialog.price)
                        clear_materials_cache()
                        union_list = self.get_union_material_list()
                        widget.configure(background="white")
                else:
    ...
    def submit_order(self):
        ...
        try:
            ...
            # ————— REMOVED: no more materials logged on order submit —————
            # self.process_new_materials()
            # self.process_new_fur_color()

            # ————— Still validate dropdown fields for completeness —————
            ...
            # ————— Create a new folder in Drive for this order —————
            ...
            if self.prod_file_paths:
                if len(self.prod_file_paths) == 1:
                    ...
                    pf_id, pf_link = create_drive_folder("Production Files", parent_id=folder_id)
                    for fp in self.prod_file_paths:
                        upload_file_to_drive(fp, pf_id)
                    prod_file_link = pf_link
            self.loading_progress.config(value=60)

            # ————— Upload print files —————
            print_file_link = ""
            if self.print_file_paths:
                prf_id, prf_link = create_drive_folder("Print Files", parent_id=folder_id)
                for fp in self.print_file_paths:
                    upload_file_to_drive(fp, prf_id)
                print_file_link = prf_link
            self.loading_progress.config(value=65)

            # ————— Write headers & data to the sheet —————
            ...
            for field, col_name in mapping.items():
                ...
                if idx is not None:
                    ...
                    messagebox.showwarning("Warning", f"'{col_name}' header not found.")
            ...
            # Copy down formulas: Stage, Ship Date, EMB Backing, Preview, Stitch Count
