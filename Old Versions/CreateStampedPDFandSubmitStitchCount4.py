...
def draw_top_right_box(pdf, order_number, spreadsheet, w, h, s):
    box_w, box_h = 60, 120
    x0, y0 = w - box_w - 10, 10
    pdf.set_line_width(1); pdf.set_draw_color(0,0,0)
    ...
    pdf.set_font('Arial','',8); pdf.set_fill_color(*rgb)
    pdf.rect(content_x, starts[2], inner_w, rows[2], style='F')
    pdf.set_text_color(*contrast); pdf.set_xy(content_x, ys[2])
    pdf.cell(inner_w, rows[2], fur, 0, 2, 'C')

    pdf.set_fill_color(*rgb)
    pdf.rect(content_x, starts[3], inner_w, rows[3], style='F')
    pdf.set_text_color(*contrast); pdf.set_xy(content_x, ys[3])
...
def shrink_page_and_stamp_horizontal_qrs(original_pdf_path, order_number, spreadsheet, margin_pts=72):
    ...
    try:
        ...
        # uniform scale: at most 10% shrink, but also clear stamps
        max_w = w - left_margin - right_margin
        max_h = h - top_margin - bottom_margin
        scale = min(0.90, max_w / w, max_h / h)

        # translate so content is flush left and above bottom_margin
        tx = left_margin
        ty = bottom_margin
        ...
        # Right-side QR column (start below the info box)
        x0 = w - box_w - pad
        y_start = pad + box_h + pad + label_h
        ...
        # 3) Bottom notes box: pull from Google Sheet 'Notes' column
        note_pad = 10
        ...
        pdf.set_line_width(1); pdf.set_draw_color(0,0,0)
        pdf.rect(note_x, note_y, note_w, note_h)
