# CreateStampedPDFandSubmitStitchCount3.py
...
def draw_top_right_box(pdf, order_number, spreadsheet, w, h, s):
    ...
    pdf.cell(inner_w, rows[1],
             f"QTY: {int(get_order_quantity(order_number, spreadsheet))}",
             0, 2, 'C')

    fur = re.sub(r"\s*fur\s*$", "", get_fur_color(order_number, spreadsheet),
    ...
    pdf.cell(inner_w, rows[5],
             f"Ship: {get_due_date(order_number, spreadsheet)}",
             0, 2, 'C')
...
def shrink_page_and_stamp_horizontal_qrs(original_pdf_path, order_number, spreadsheet, margin_pts=72):
    ...
    try:
        with open(original_pdf_path, 'rb') as f:
            ...
            w = float(page0.mediaBox.getWidth())
            h = float(page0.mediaBox.getHeight())
            size, label_h, pad = 54, 10, 10
            extra = 2 * (label_h + size + pad)
            half = extra / 2
            scale = (h - extra) / h
            ...
            for idx in range(5):
                ...
                if idx == 0:
                    ...
                    tmp = f"qr_top_{idx}.png"
                    img.save(tmp)
                    pdf.set_xy(x, y_lbl)
                    pdf.cell(size, label_h, "Machine Start", 0, 2, 'C')
                    pdf.image(tmp, x, y_img, w=size, h=size)
                    os.remove(tmp)
                else:
                    pdf.set_draw_color(200, 200, 200)
                    pdf.rect(x, y_img, size, size)

            # Bottom QR row
            bottom_labels = ['Fur List', 'Cut List', 'Print List', 'Embroidery List', 'Shipping']
            y_img2 = h - pad - size
            y_lbl2 = y_img2 - label_h
            for idx, label in enumerate(bottom_labels):
                ...
                qr2.add_data(payload2)
                qr2.make(fit=True)
                img2 = qr2.make_image().convert('1', dither=Image.NONE)
                tmp2 = f"qr_bot_{idx}.png"
                img2.save(tmp2)
                pdf.set_xy(x2, y_lbl2)
                pdf.cell(size, label_h, label, 0, 2, 'C')
                pdf.image(tmp2, x2, y_img2, w=size, h=size)
                os.remove(tmp2)
            ...
            pg = writer.getPage(0)
            pg.merge_page(overlay.getPage(0))
