...
def queue_design():
    ...
    while time.time() - start < max_wait:
        try:
            app_conn = Application(backend="uia").connect(title_re=EMB_WINDOW_TITLE, timeout=1)
            win      = app_conn.window(title_re=EMB_WINDOW_TITLE)
        ...
        return f"Error: design window not loaded in {max_wait}s", 500

    # 5) Minimize and send Shift+Alt+Q in background
    try:
        win.minimize()
    except:
        pass
...
def open_and_handle_url(url):
    ...
    # wait up to 5s for "Success" in page
    start = time.time()
    success = False
    while time.time() - start < 5:
        ...
        time.sleep(0.5)
    if success:
        # flash green background then close
        for _ in range(4):
        ...
        # continuous red flash until closed manually
        while True:
...
def on_key_event(event):
    ...
    if key.lower() in ('enter','return'):
        ...
        print(f"DEBUG ▶ Full scan buffer: {candidate}")
