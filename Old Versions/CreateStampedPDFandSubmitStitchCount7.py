...
# ─── Embedded Queue Service Imports ───
...
def queue_design():
    ...
    # 3) Launch minimized so it never steals focus
    #    Use a single shell string—this avoids the "\\ not found" error
    subprocess.Popen(
        f'start "" /min "{emb_path}"',
        shell=True
    )

    # 4) Poll up to 5 minutes for EmbroideryStudio to appear
    ...
    while True:
        try:
            ...
            if time.time() - start > max_wait:
                return f"Error: EmbroideryStudio not found in {max_wait}s", 500
            time.sleep(interval)

    # 5) Send Shift+Alt+Q in the background
    try:
...
def shrink_page_and_stamp_horizontal_qrs(original_pdf_path, order_number, spreadsheet, margin_pts=72):
    ...
    try:
        ...
        # Top QR row – evenly spaced between pad and the info‑box
        top_size   = int(size * 0.75)
        pad        = 10
        box_w      = 60  # same as info-box width
        left_bound  = pad
        ...
        for idx in range(5):
            ...
            if idx == 0:
                ...
                # Queue Design QR (only this slot)
                queue_url = (
                    f"http://192.168.1.24:5001/queue?"
                    f"file={urllib.parse.quote(order_number)}"
                )
                qr  = qrcode.QRCode(error_correction=qrcode.constants.ERROR_CORRECT_L,
                                    box_size=6, border=0)
                ...
                # leave other grey boxes untouched
        ...
        # Right‑side QR column (unchanged)...
        x0 = w - box_w - pad
...
if __name__ == "__main__":
    ...
    # (Optional) Silence Flask/Werkzeug dev‑server warning
    import logging
    log = logging.getLogger('werkzeug')
    log.setLevel(logging.ERROR)

    # 2) Start embedded Flask queue‑service on port 5001
    ...
    # 3) Begin watching the folder for new PDFs
    ...
    # 4) Hook keyboard for QR scanning and wait
