...
def _render_pdf_page_to_image(pdf_path, page_index=0, zoom=2.0):
    """
    Uses PyMuPDF to render a PDF page to a PIL image.
    zoom=2.0 means ~144dpi; adjust if you want sharper.
    Returns (PIL.Image, page_width_pts, page_height_pts, scale_x, scale_y)
    """
    import fitz  # PyMuPDF
    ...
    sy = pix.height / ph
    doc.close()
    return img, pw, ph, sx, sy

def prompt_stops_interactive(pdf_path: str, seq_count: int):
    """
    Opens a Tk window with a preview of page 1 and checkboxes aligned to detected
    sequence rows. Clicking a checkbox toggles a red triangle on the preview.
    Returns a sorted list of selected sequence numbers.
    If detection fails, falls back to a simple vertical list in the same window.
    """
    import tkinter as tk
    from tkinter import ttk
    from PIL import ImageTk

    # Detect row positions on the PDF
    row_pos = _detect_sequence_rows_with_pdfplumber(pdf_path, seq_count)  # {n: (x0, y_pts)}
    # Render the PDF page to an image
    img, pw, ph, sx, sy = _render_pdf_page_to_image(pdf_path, page_index=0, zoom=2.0)

    root = tk.Tk()
    root.title("Select Stops (click checkboxes)")

    # Left: controls; Right: PDF canvas
    left = ttk.Frame(root)
    left.pack(side="left", fill="y", padx=10, pady=10)
    right = ttk.Frame(root)
    right.pack(side="right", fill="both", expand=True, padx=10, pady=10)

    # Scrollable canvas for PDF
    ...
    # Draw a faint guide line near where we'll place triangles (left margin of the table)
    default_x_px = int(36 * sx)  # fallback x: 0.5" from left
    triangle_items = {}          # seq -> canvas item id (or None)
    check_vars = {}              # seq -> BooleanVar

    # Compute per-sequence checkbox/triangle positions
    # If we found actual positions, use those. Otherwise, spread evenly.
    seq_positions_px = {}
    if row_pos:
        # Use detected per-row y positions
        # Triangles: a bit left of the sequence number x (subtract ~10px)
        for n in range(1, seq_count + 1):
            if n in row_pos:
                x0_pts, y_pts = row_pos[n]
                x_px = int(x0_pts * sx) - 12
                y_px = int(y_pts * sy)
                x_px = max(8, x_px)
                seq_positions_px[n] = (x_px, y_px)
    else:
        # fallback: evenly space down the page margin
        top = int(120 * sy)
        step = int(24 * sy)
        for n in range(1, seq_count + 1):
            seq_positions_px[n] = (default_x_px, top + (n - 1) * step)

    # Draw helper: triangle
    def draw_triangle(x, y, size=10):
        # Upward triangle (x,y is baseline-left)
        return canvas.create_polygon(
            x, y, x + size, y, x + size/2, y - size,
            fill="red", outline=""
        )

    # Toggle handler: create/remove triangle for seq
    def toggle_seq(n, var):
        ...
        # if checked, draw new triangle
        if var.get():
            x, y = seq_positions_px[n]
            triangle_items[n] = draw_triangle(x, y, size=12)

    # Build left controls: header + checkbox list + buttons
    ...
    # Checkboxes (grid 2 columns to avoid a tall column)
    cols = 2
    for i in range(seq_count):
        n = i + 1
        v = tk.BooleanVar(value=False)
        check_vars[n] = v
        cb = ttk.Checkbutton(list_frame, text=f"Seq {n}", variable=v, command=lambda n=n, v=v: toggle_seq(n, v))
        r, c = divmod(i, cols)
        cb.grid(row=r, column=c, sticky="w", padx=6, pady=4)

    # Quick picks row
    quick = ttk.Frame(left)
    quick.pack(anchor="w", pady=(6,6))
    ...
    btns = ttk.Frame(left)
    btns.pack(anchor="e", pady=(8,0), fill="x")
...
def prompt_stops_for_sequences(seq_count: int):
    """
    Shows a Windows MessageBox to confirm we can show UI.
    Then (for now) uses a console prompt to gather stops: '1,3,6'.
    Returns a sorted list of ints (1..seq_count).
    """
    print(f"[Stops] Entering prompt for {seq_count} sequences...", flush=True)

    # 0) Always try to show a debug MessageBox so we know UI is possible
    _debug_msgbox("JR & Co — Stops (debug)",
                  f"About to collect stops for {seq_count} sequences.\n"
                  f"If no GUI appears next, type them in the console as '1,3,6'.")

    # 1) Console fallback (bulletproof and fast)
    try:
        print(f"[Stops] Console — enter numbers 1..{seq_count} separated by commas, or press Enter for none.", flush=True)
        raw = input("Stops> ").strip()
        if not raw:
            print("[Stops] No stops selected (console).", flush=True)
            return []
        ...
        print(f"[Stops] Console selection: {vals}", flush=True)
        return vals
    except Exception as e:
        print(f"[Stops] Console fallback failed: {e}", flush=True)
        return []
...
def shrink_page_and_stamp_horizontal_qrs(original_pdf_path, order_number, spreadsheet, margin_pts=72, stops=None):
    ...
    if stops:
        # Re-detect row positions in PDF points, then draw triangles near those Y's.
        row_pos = _detect_sequence_rows_with_pdfplumber(original_pdf_path, max(stops))
        # Fallback x if we can't find a specific x per row
        fallback_x = pad  # 0.5" from left border of overlay
        pdf.set_fill_color(200, 0, 0)

        def _triangle_pts(xc, yc, s=8):
            return [(xc, yc), (xc + s, yc), (xc + s/2, yc - s)]

        for n in stops:
            # pick x from detected x0 if available, else fallback
            if n in row_pos:
                x0_pts, y_pts = row_pos[n]
                # place triangle just left of the detected sequence number
                x_pdf = max(pad, x0_pts - 10)  # 10 pt to the left of the number
                y_pdf = y_pts
            else:
                x_pdf = fallback_x
                # distribute vertically if no detection (very rare)
                # pack them 14 pt apart from top padding
                y_pdf = pad + 20 + (n-1)*14

            try:
                pdf.polygon(_triangle_pts(x_pdf, y_pdf, s=8), style='F')
            except Exception:
                # fallback: draw a small square if polygon isn't supported
                pdf.rect(x_pdf, y_pdf - 8, 8, 8, style='F')
    ...
    while True:
        try:
            ...
            for file in items:
                ...
                try:
                    print(f"[Stops] Showing stop selection dialog for {order}...", flush=True)
                    stops = prompt_stops_for_sequences(len(usage))
                except Exception as e:
                    print(f"[Stops] Prompt failed ({e}); proceeding with no stops.", flush=True)
                    stops = []
                ...
                print(f"[Stops] Stamping with stops: {stops}", flush=True)
                shrink_page_and_stamp_horizontal_qrs(local_path, order, sheet_spreadsheet, stops=stops)
