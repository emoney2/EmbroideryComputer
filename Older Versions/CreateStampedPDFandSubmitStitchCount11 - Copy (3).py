...
try:
    ...
    print("[Thumbs] psd-tools not available â†’ PSD/PSB previews will use placeholders.", _e)
...
def _detect_max_sequence(pdf_path: str) -> int:
    """
    Best-effort: scan page 1 for the largest integer token; cap to something sane.
    Used to size the checkbox list in the Stops UI if a count isnâ€™t known.
    """
    try:
        import pdfplumber
        with pdfplumber.open(pdf_path) as pdf:
            ...
            for w in words:
                ...
                if t.isdigit():
                    ...
                    if 1 <= n <= 999:
                        ints.append(n)
            return min(max(ints) if ints else 0, 120)
    except Exception:
        return 0
...
def prompt_stops_interactive(pdf_path: str, seq_count: int):
    ...
    def _show(root):
        ...
        # Draw and keep strong refs so Tk doesnâ€™t GC it
        ...
        # Use the header-anchored locator that scans: "Stop Sequence:" + "# Color St. Code Name"
        positions, st_left = _locate_stop_sequence_rows(pdf_path, max_rows=seq_count)
        ...
        if positions and (st_left is not None):
            for n in range(1, seq_count + 1):
                ...
                # Canvas triangle is centered horizontally; offset by half the pixel size so the TIP lands at x_tip
                x_px = int(x_tip_pts * sx) + (ui_tri_size // 2)
        ...
        def draw_triangle(x, y, size=14):
            # LEFT-pointing triangle (90Â° CCW), centered at (x, y)
            half = size // 2
        ...
        ttk.Label(left, text=f"Detected sequences: {seq_count}", font=("Segoe UI", 11, "bold")).pack(anchor="w", pady=(0,6))
        ...
        for i in range(seq_count):
            ...
            # If row 1, draw its triangle immediately so it shows on the preview
            if n == 1 and (n in seq_positions_px):
                toggle_seq(n, v)

        quick = ttk.Frame(left); quick.pack(anchor="w", pady=(6,6))
        def _all():  [check_vars[n].set(True)  or toggle_seq(n, check_vars[n]) for n in range(1, seq_count+1)]
        def _none(): [check_vars[n].set(False) or toggle_seq(n, check_vars[n]) for n in range(1, seq_count+1)]
        ...
        def _finish(vals):
            ...
            # NEW: persist the exact UI positions we used (PDF points, top-down) into a sidecar
            try:
                import os, json
                ui_tri_size = 14  # keep in sync with the preview triangle size
                marks_pdf = {}
                for n, (cx_px, cy_px) in (seq_positions_px or {}).items():
                    # seq_positions_px stores CENTER X in pixels; convert back to left-tip in PDF points
                    x_tip_pts = (float(cx_px) - (ui_tri_size // 2)) / float(sx)
                    y_pts     = float(cy_px) / float(sy)
                    marks_pdf[int(n)] = [x_tip_pts, y_pts]
                sidecar = os.path.splitext(pdf_path)[0] + ".marks.json"
                with open(sidecar, "w", encoding="utf-8") as f:
                    json.dump({"pdf_path": pdf_path, "marks": marks_pdf}, f)
        ...
        def _ok():
            vals = [n for n in range(1, seq_count+1) if check_vars[n].get()]
            if not vals and seq_count >= 1:
                vals = [1]  # default to first row if user left everything unchecked
            _finish(vals)
...
def prompt_stops_for_sequences(seq_count: int, pdf_path: str | None = None):
    ...
    if pdf_path:
        print(f"[Stops] Showing visual stop selector for {os.path.basename(pdf_path)}â€¦", flush=True)
        try:
...
def _progress_modal_open(root, title="Workingâ€¦", text="Startingâ€¦"):
    import tkinter as tk
    from tkinter import ttk
    win = tk.Toplevel(root)
    win.title(title)
...
# ---- Tiny "Loading previewsâ€¦" modal (for long UI work) ----
def _open_loading_modal(root, total_count:int|None=None, text="Loading previewsâ€¦"):
    import tkinter as tk
    from tkinter import ttk
    win = tk.Toplevel(root)
...
# â”€â”€â”€ Embedded Queue Service Configuration & Route â”€â”€â”€
import subprocess
import os
import time
...
def queue_design():
    ...
    try:
        ...
        # â¬‡ NEW: close just the design window, leave the app running
        win.close()

        return "Queued âœ… and closed design window", 200
    except Exception as e:
...
def success_screen():
    ...
    # Find the oldest image in the orderâ€™s folder
    job_folder = get_job_folder(order)
    img_path, _ = find_oldest_image(job_folder)

    # Base64 encode the image (optional: if none, weâ€™ll just hide the <img>)
    img_b64 = ""
...
<body>
  <div class="wrap">
    ...
    <div class="info-col">
      ...
      <div class="mid">{design}{(" â€¢ Qty: " + str(qty)) if qty else ""}</div>
    </div>
  </div>
  <script>
...
# â”€â”€ START: Ctrl+Alt+O (open .emb) routines â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
_busy = False
...
def listen_for_ctrl_alt_o():
    """Threadâ€‘target: wait for Ctrl+Alt+O and call open_order_file."""
    while True:
        keyboard.wait(HOTKEY_O)
        open_order_file()


# â”€â”€ START: Ctrl+Alt+N (Save As PDF) routines â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _open_import_graphic_via_keystrokes_and_select(
    *, 
    ...
    """
    EXACT KEY CHORD: Alt+F â†’ wait â†’ G â†’ wait â†’ Enter (via low-level VKs),
    then: wait for file dialog â†’ navigate to folder â†’ type the selected image filename â†’ Enter.
...
def save_as_order_pdf():
    ...
    then Fileâ†’Import Graphic with the oldest PNG in that folder, then insert Product template if available.
    """
    ...
    try:
        ...
        if not ok:
            messagebox.showwarning(
                ...
                "Try again, or open File â†’ Import manually."
            )
...
def listen_for_ctrl_alt_n():
    ...
    keyboard.wait()
# â”€â”€ END: Ctrl+Alt+N (Save As PDF) routines â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _active_order_from_wilcom() -> str | None:
    ...
    Accepts formats like: '... â€“ [$VP Tajima TBF] - [email]' or '... â€“ [172] - [...]'
...
def connect_google_sheet(sheet_name, tab_name=TAB_NAME):
    # â–¶ Explicitly load the JSON key so thereâ€™s no ambiguity
        ...
        for n in stops:
            try:
                if n in marks:
                    ...
                    # Apply the exact same page transform we applied to the original page:
                    # x' = x*scale + tx;  y' = y*scale + ty   (FPDF uses a top-left origin with y down)
                    x_pdf = (x_tip_pts * scale) + tx
                    y_pdf = (y_mid_pts * scale) + ty
                else:
...
def _locate_stop_sequence_rows(pdf_path: str, max_rows: int = 200):
    ...
    Strategy:
      ...
      - For each row (1., 2., ...), compute a y_center between rows (biased higher so it doesnâ€™t look low).
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for idx, t in enumerate(hdr_texts):
            if t in ("st", "st.", "stitch"):
                ...
                break
        if st_left is None:
            # Fallback: use right edge of header line â€” better than nothing
        ...
        # 4) For each row, compute a y_center biased higher so it doesnâ€™t look low
        positions = {}
...
def get_stop_mark_points_pdfspace(pdf_path: str, max_seq: int, tip_inset_pts: float = 3.0) -> dict[int, tuple[float, float]]:
    """
    Return {n -> (x_tip_pts, y_between_pts)} in PDF POINTS (top-down):
      - x_tip_pts: tip just LEFT of the 'St.' column inside the Stop Sequence box
      - y_between_pts: exact midpoint between row n and row n+1
    Prefers positions saved by the UI sidecar if present; otherwise falls back to parsing.
    """
    import os, json

    # 1) Try to use the exact UI positions from a sidecar JSON (if present)
    sidecar = os.path.splitext(pdf_path)[0] + ".marks.json"
    try:
        if os.path.exists(sidecar):
            with open(sidecar, "r", encoding="utf-8") as f:
                data = json.load(f)
            # sidecar stores { n: [x_tip_pts, y_pts] } in PDF points (top-down)
            marks_from_ui = {
            ...
            if marks_from_ui:
                # Keep only rows up to max_seq
                return {n: marks_from_ui[n] for n in sorted(marks_from_ui) if 1 <= n <= int(max_seq)}
    except Exception as e:
        print(f"[get_stop_mark_points_pdfspace] sidecar read failed: {e}", flush=True)

    # 2) Fallback: parse Stop Sequence header/rows directly
    try:
        positions, st_left = _locate_stop_sequence_rows(pdf_path, max_rows=int(max_seq))
    except Exception as e:
        print(f"[get_stop_mark_points_pdfspace] locator failed: {e}", flush=True)
        return {}

    if not positions or st_left is None:
        return {}

    x_tip = float(st_left) - float(tip_inset_pts)
    out: dict[int, tuple[float, float]] = {}
    for n in range(1, int(max_seq) + 1):
        y = positions.get(n)
        if y is None:
            continue
        try:
            out[int(n)] = (x_tip, float(y))
        except Exception:
            continue
    return out
...
def stamp_triangles_only(original_pdf_path: str, order_number: str, stops: list[int],
    ...
    try:
        ...
        sidecar = os.path.splitext(original_pdf_path)[0] + ".marks.json"
        if os.path.exists(sidecar):
            try:
                with open(sidecar, "r", encoding="utf-8") as f:
                    data = json.load(f)
                # sidecar: { n: [x_tip_pts, y_pts] } in PDF points (top-down)
                marks_from_ui = {int(k): (float(v[0]), float(v[1])) for k, v in (data.get("marks") or {}).items()}
                print(f"[TrianglesOnly] Using UI sidecar marks: {len(marks_from_ui)} entries", flush=True)
            except Exception as e:
                print(f"[TrianglesOnly] Sidecar read failed: {e}", flush=True)
...
class StopsQueueHandler(FileSystemEventHandler):
    ...
    def _handle_new_pdf(self, p: str):
        try:
            ...
            if usage:
                seq_max = min(len(usage), 150)
            else:
                try:
                    positions = _detect_sequence_rows_with_pdfplumber(p, 220)
                    seq_max = min(max(positions.keys()) if positions else 30, 150)
                except Exception:
                    seq_max = 30
...
class ProdToStopsInterceptor(FileSystemEventHandler):
    ...
    def on_created(self, event):
        ...
        try:
            ...
            print(f"[Intercept] Moving {base} â†’ Stops Queue")
            shutil.move(p, dst)
        except Exception as e:
...
class PDFHandler(FileSystemEventHandler):
    def on_created(self, event):
        ...
        # DEBUG â€” prove we are here and can show UI
        _debug_msgbox("JR & Co â€” Stops checkpoint", f"Detected usage rows: {len(usage)} for order {order}")
        ...
        # MOVE the justâ€‘processed PDF into â€œprocessed/â€ so Watchdog wonâ€™t see it again
        import shutil, os
        ...
        try:
            ...
            for file in items:
                ...
                if not os.path.exists(stamped_path):
                    time.sleep(0.5)
                    if not os.path.exists(stamped_path):
                        print(f"âš ï¸ Stamped file not found yet for {name}. Skipping this cycle.")
                        continue
                ...
                print(f"âœ… Uploaded stamped version to Stamped PS: {uploaded['id']}")
                ...
                if order_folders:
                    ...
                    print(f"âš ï¸ No subfolder found for order '{order_folder_name}' in Orders folder.")
...
def _pick_files_in_folder_with_root(root, initialdir: Path):
    ...
    loading_win, loading_pb, set_progress = _open_loading_modal(root, total_count=len(files), text="Loading previewsâ€¦")
    try:
...
def _run_pdf_presentation_picker_on_root(root):
    ...
    if not files:
        # (User canceled in reorder dialog â€” keep earlier guard behavior if you like)
        try:
    ...
    def build():
        ...
        run_on_tk_thread(lambda root: _progress_modal_open(root, title="Building PDFâ€¦", text="Preparingâ€¦ 0%"))
...
def _export_single_page_pdf(ps, in_path: Path, out_pdf_path: Path):
    ...
    # Lower JPEG quality to shrink pages. Range 0â€“12; 6â€“8 is a sweet spot.
...
def _build_pdf_presentation(parent: Path, paths: list[Path]) -> Path:
    ...
    try:
        ...
        for idx, p in enumerate(paths, start=1):
            ...
            run_on_tk_thread(lambda root, i=idx-1: _progress_modal_update(root, _pct_export(i, total_pages), f"Exportingâ€¦ {i}/{total_pages}"))
            _export_single_page_pdf(ps, p, tmp)
            temp_pdfs.append(tmp)
            run_on_tk_thread(lambda root, i=idx: _progress_modal_update(root, _pct_export(i, total_pages), f"Exportingâ€¦ {i}/{total_pages}"))

        # --- MERGE ---
        print(f"[PDF-Pres] Merging {len(temp_pdfs)} pagesâ€¦")
        run_on_tk_thread(lambda root: _progress_modal_update(root, 90.0, "Merging pagesâ€¦"))
        merger = PdfMerger()
        ...
        run_on_tk_thread(lambda root: _progress_modal_update(root, 95.0, "Compressingâ€¦"))
...
def _ghostscript_compress(in_pdf: Path, out_pdf: Path, quality: str = "ebook") -> tuple[bool, str | None]:
    ...
    qmap = {
        ...
        "ebook": "/ebook",      # ~150 dpi  â† good default
...
def _run_pdf_presentation_picker():
    ...
    try:
        ...
        # Re-enable scanner/clipboard when weâ€™re done
        PDF_BUILD_ACTIVE = False
        SCANNER_ENABLED = True
...
def clipboard_listener():
    ...
    while True:
        try:
            ...
            # At this point it's a valid JRCO QR link â†’ handle it
...
def get_active_job_number() -> str:
    ...
    for b in blocks:
        s = b.strip()
        tok = re.split(r"\s{2,}| - ", s)[0].strip()      # "172      Tajima TBF" â†’ "172"
...
def on_control_alt_p():
    ...
    Ctrl+Alt+P:
      ...
      2) Do NOT wait for any PDF here. Let the local watchers (below) handle:
         Prep â†’ Production (intercept) â†’ Stops Queue (visual UI) â†’ stamp â†’ back to Production.
    """
    print("[Control+Alt+P] Triggered.")
    on_control_alt_t()

    # Best-effort log of the order weâ€™ll be working on (optional)
...
if __name__ == "__main__":
    ...
    try:
        ...
        ENABLE_PROD_TO_STOPS = False  # â† keep False now that controlaltp2.py sends to Stops Queue directly
        ...
        # Start Stops Queue watcher: opens checkbox UI â†’ stamp triangles â†’ copy back to Production
        stops_dir = r"G:\My Drive\Stops Queue"
        os.makedirs(stops_dir, exist_ok=True)
    ...
    print("[Startup] Hotkey registered: Ctrl+Alt+T â†’ on_control_alt_t()")
    keyboard.add_hotkey("ctrl+alt+p", on_control_alt_p)
    print("[Startup] Hotkey registered: Ctrl+Alt+P â†’ on_control_alt_p()")

    keyboard.add_hotkey("ctrl+alt+d", lambda: run_on_tk_thread(_run_pdf_presentation_picker_on_root))
    print("[Startup] Hotkey registered: Ctrl+Alt+D â†’ Tk-dispatched PDF picker")
